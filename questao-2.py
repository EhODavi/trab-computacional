...
def desenhar(solucao, pontos, n):
    ...
    for i in range(len(solucao) - 1):
        G.add_edge(solucao[i].id, solucao[i + 1].id, color='blue', weight=1)
...
def main():
    if len(sys.argv) != 2:
        print("Erro, use: python questao-2.py arquivo")
        exit(-1)
